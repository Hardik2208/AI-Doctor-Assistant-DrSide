...
export default function ChatRoom() {
  ...
  const isDoctor = user.toLowerCase().includes('doctor') || user.toLowerCase().includes('dr');
        ...
        {messages.length === 0 ? (
          ...
          <>
            {messages.map((msg, idx) => {
              ...
              const showAvatar = idx === 0 || messages[idx - 1].user !== msg.user;
              
              return (
                <div key={idx} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-4`}>
                  <div className={`flex max-w-xs lg:max-w-md ${isOwn ? 'flex-row-reverse' : 'flex-row'} items-end space-x-2`}>
                    {!isOwn && showAvatar && (
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-semibold ${
                        msg.user.toLowerCase().includes('doctor') ? 'bg-blue-500' : 'bg-green-500'
                      }`}>
                        {msg.user.toLowerCase().includes('doctor') ? 'Dr' : msg.user.charAt(0).toUpperCase()}
                      </div>
                    )}
